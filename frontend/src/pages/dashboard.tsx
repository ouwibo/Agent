...
type LiveStats = { ok: boolean; uptimeSec?: number; requestsHandled?: number; serverKeys?: Record<string, boolean>; availableProviders?: string[]; usersCount?: number; messagesCount?: number }
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden flex selection:bg-primary selection:text-black">
      ...
      <AnimatePresence>{sidebarOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-20 md:hidden" onClick={() => setSidebarOpen(false)} />}</AnimatePresence>

      <aside className={`fixed md:relative inset-y-0 left-0 z-30 md:z-20 w-64 border-r border-primary/20 bg-black/95 backdrop-blur-md flex flex-col transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-5 border-b border-primary/20 flex items-center justify-between">
          <Link href="/">
            <div className="flex items-center gap-2.5 cursor-pointer group">
              ...
              <h2 className="text-xl font-bold text-white tracking-widest drop-shadow-[0_0_5px_rgba(0,255,65,0.8)] group-hover:text-primary transition-colors">OUWIBO<span className="text-primary animate-pulse">_</span></h2>
            </div>
          </Link>
          <button className="md:hidden text-primary/60 hover:text-primary transition-colors" onClick={() => setSidebarOpen(false)}><X className="w-5 h-5" /></button>
        </div>
      ...
      <main className="flex-1 flex flex-col relative z-10 h-screen overflow-hidden min-w-0">
        <header className="h-14 border-b border-primary/15 bg-black/60 backdrop-blur-md flex items-center justify-between px-4 md:px-6 shrink-0">
          <div className="flex items-center gap-3">
            <button className="md:hidden text-primary/60 hover:text-primary transition-colors" onClick={() => setSidebarOpen(true)}><Menu className="w-5 h-5" /></button>
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <Link href="/agent" className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary/15 border border-primary/30 text-primary text-xs font-medium hover:bg-primary/25 transition-all"><Bot className="w-3 h-3" /><span className="hidden sm:inline">Launch Agent</span><span className="sm:hidden">Agent</span></Link>
            <div className="flex items-center gap-2 px-3 py-1.5 bg-primary/8 border border-primary/20 text-xs rounded-xl font-mono"><div className="w-2 h-2 rounded-full bg-primary animate-pulse" /><span className="text-primary/70 hidden sm:inline">ONLINE</span></div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="border border-white/8 bg-black/40 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-white/8 bg-white/[0.02] flex items-center justify-between"><span className="font-mono font-bold text-sm text-white/70 tracking-wider">&gt; QUICK_ACTIONS</span><Zap className="w-4 h-4 text-primary/40" /></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }} className="lg:col-span-2 border border-white/8 bg-black/40 rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-white/8 bg-white/[0.02] flex items-center justify-between"><span className="font-mono font-bold text-sm text-white/70 tracking-wider">&gt; ACTIVE_AGENTS</span><span className="text-[10px] font-mono text-white/30">{agents.filter(a => a.status !== 'IDLE').length} RUNNING</span></div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} className="border border-white/8 bg-black/40 rounded-xl overflow-hidden flex flex-col min-h-56 lg:min-h-0">
              <div className="px-4 py-3 border-b border-white/8 bg-white/[0.02] flex items-center justify-between"><span className="font-mono font-bold text-sm text-white/70 tracking-wider">&gt; SYS_LOGS</span><Terminal className="w-3.5 h-3.5 text-white/20" /></div>
              <div className="flex-1 p-3 overflow-y-auto text-[11px] space-y-1.5 flex flex-col justify-end font-mono">
                <AnimatePresence initial={false}>{logs.map((log, i) => (<motion.div key={log + i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} className={log.includes('[SYS]') ? 'text-primary/60' : log.includes('[TOOL]') ? 'text-blue-400/70' : log.includes('[AGENT]') ? 'text-white/50' : log.includes('[NET]') ? 'text-purple-400/70' : 'text-white/30'}>{log}</motion.div>))}</AnimatePresence>
